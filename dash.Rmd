...
renderUI({
  ...
  if (input$same) {
    ...
    s2 <- selectInput(inputId = 'titleF',
            ...
            choices = titles,
            width = '100%')
    tagList(s1,s2, HTML("<div>First select one of the six subjects then choose if you want two views of the same record, or each view to show a seperate record.</div>\n<div>This app is a viwer for some of the records used in my thesis. It shows all the relavent information provided about two reords, along with the models predictions.</div>"))
  }
})
...
renderUI({
  ...
  s = paste(s, '<div><b>', 'Full Model Prediction: ','</b><i>', row2()[['pred_part']], '</i> with probabily of being computational: ', sprintf('%.1f', row2()[['probPos_partial']] * 100) , '%</div>', sep = '')
  HTML(s)
})
